...
export default function MarketingInsights() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (isDemo) {
        setMarketData(getStaticMarketData());
        ...
        // Load real data from database (generated by MarketResearchAgent)
        try {
          const insights = await SearchService.getMarketInsights(currentSearch.id);
          if (insights) {
            setMarketData({
              tam: insights.tam_data,
              sam: insights.sam_data, 
              som: insights.som_data
            });
            setCompetitorData(insights.competitor_data || []);
            setTrends(insights.trends || []);
            setSources(insights.sources || []);
            setResearchSummary(insights.opportunities?.summary || '');
            setMethodology(insights.research_methodology || '');
          } else {
            // Agent data not ready yet - show empty state
            console.log(`No market insights found for search ${currentSearch.id}`);
            setMarketData(getEmptyMarketData());
            setCompetitorData([]);
            setTrends([]);
            setSources([]);
            setResearchSummary('');
  ...
  const getEmptyMarketData = () => ({
    tam: { value: '$0', growth: '0%', description: 'Total Addressable Market' },
    sam: { value: '$0', growth: '0%', description: 'Serviceable Addressable Market' },
    som: { value: '$0', growth: '0%', description: 'Serviceable Obtainable Market' }
  });
